...
Usage:
    caffeinate -i python3 experiments/certify_sst2_bert.py --layers 1
    caffeinate -i python3 experiments/certify_sst2_bert.py --layers 2
    caffeinate -i python3 experiments/certify_sst2_bert.py --layers 3 --skip-diff
    caffeinate -i python3 experiments/certify_sst2_bert.py --bits 4 --n-samples 5
...
def load_checkpoint(num_layers=None):
...
def load_sst2_samples(model, state, vocab, n=10, max_len=32):
    """Load correctly classified SST-2 validation samples."""
...
def main():
    ...
    parser.add_argument("--split-budget", type=int, default=0,
                        help="Branch-and-bound leaf budget (0 disables; 1 = no split; 16 typical)")
    parser.add_argument("--split-strategy", choices=["largest", "gradient"],
                        default="largest")
    ...
    if args.split_budget > 1:
        from boundlab.diff.certify_split import certify_differential_split
        def _split_method(m1, m2, c, e):
            return certify_differential_split(
                m1, m2, c, e,
                budget=args.split_budget,
                split_strategy=args.split_strategy,
            )
        methods.append((f"Diff-Split(b={args.split_budget})", _split_method))

    print(f"\n{'='*80}")
    print(f"CERTIFY: Trained SST-2 BERT ({args.layers} layers) vs Quantized")
    print(f"{'='*80}")
    ...
    for bits in args.bits:
        ...
        # Summary table for this bit level
        ...
        for name, _ in methods:
            ...
            if r:
                ...
                avg_width = sum(w for _, _, w in r) / len(r)
