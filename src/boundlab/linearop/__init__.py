...
from ._hardmarddot import HardmardDot
from ._shape import (
    ReshapeOp,
    FlattenOp,
    UnflattenOp,
...
class ScalarMul(LinearOp):
    ...
    def __mul__(self, other: float) -> "ScalarMul":
        """Scale this ScalarMul by another scalar."""
        if isinstance(other, (int, float)):
            return ScalarMul(self.scalar * other, self.input_shape)
        return NotImplemented

    def __rmul__(self, other: float) -> "ScalarMul":
        """Scale this ScalarMul by another scalar."""
    ...
    def __add__(self, other):
        if isinstance(other, ScalarMul):
            return ScalarMul(self.scalar + other.scalar, self.input_shape)
    ...
    def is_identity(self) -> bool:
        """Check if this ScalarMul is an identity operation (scalar == 1)."""
