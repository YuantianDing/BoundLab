...
def _abcrown_query_lb(
    ...
    """Query lower bounds for all output classes using only lower=True VNN-LIB queries.
...
class BatchedMaskedModel(nn.Module):
    """Batched wrapper around MaskedModel for torch.onnx.export (abcrown needs batch dim)."""
...
def _bound_width_reasons(e: expr.Expr) -> dict[str, Tensor]:
    """Return detached width-reason tensors for an expression."""
    return {
        reason: value.detach()
...
def _merge_reason_breakdowns(
    ...
    """Merge per-case width reasons using max contribution per output."""
    if merged is None:
...
def _build_batched_masked(vit, mask: Tensor, cache: dict[bytes, object]):
    """Build and cache a BatchedMaskedModel wrapper keyed by the binary mask."""
    key = _mask_cache_key(mask)
    if key not in cache:
        cache[key] = BatchedMaskedModel(MaskedModel(vit, mask).eval())
    return cache[key]
...
def main():
    ...
    try:
        ...
        print("[warn] auto_LiRPA not found; abcrown method will be skipped", flush=True)
    ...
    al_cache: dict[bytes, object] = {}
    mask_full = torch.ones(17, 64)
    gm_full = _export_masked_onnx(vit, mask_full, onnx_cache)
    bm_full = _build_batched_masked(vit, mask_full, al_cache) if _have_autolirpa else None
    ...
    for i, (img, label) in enumerate(samples):
        ...
        for name, fn in methods:
            ...
            for case_kept in cases:
                ...
                best_breakdown = _merge_reason_breakdowns(
            ...
            print(f"    {'reasons':<15} "
                    f"{_format_reason_breakdown(best_breakdown or {}, width)}")
            # except Exception as e:
            #     elapsed = time.perf_counter() - t0
                # print(f"    {name:<15} FAILED: {e} ({elapsed:.1f}s)")
        # abcrown: independent auto_LiRPA bounds on each model, then subtract
        if _have_autolirpa:
            t0 = time.perf_counter()
            best_d_ub = None
            best_d_lb = None
            try:
                for case_kept in cases:
                    ...
                    for p in case_kept:
                        mask_pruned[p + 1] = 1.0
                    bm_pruned = _build_batched_masked(vit, mask_pruned, al_cache)
                ...
                width = (best_d_ub - best_d_lb).mean().item()
                all_results["abcrown"].append((bound, elapsed, width))
    ...
    int_width = None
    summary_methods = [name for name, _ in methods] + ["abcrown"]
