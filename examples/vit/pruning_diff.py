...
from boundlab.diff.op import softmax_pruning
...
class Attention(nn.Module):
    ...
    def forward(self, x: Tensor, token_mask: torch.Tensor) -> Tensor:
        # x: (N, D)
        n = x.shape[0]
        ...
        mask = token_mask.view(1, 1, -1).expand_as(dots) # (1, 1, N) → (h, N, N)
        attn = softmax_pruning(mask, dots, dim=-1)  # (h, N, N)
...
if not (DIRPATH / "onnx").is_dir():
    (DIRPATH / "onnx").mkdir()

def mnist_vit_pruning(
    depth: int = 1,
    ...
    """
    assert depth in (1, 3)
    path = DIRPATH / "onnx" / f"mnist_vit_{depth}_{layer_norm_type}.onnx"
    if path.is_file():
        return path
    ...
    exported = onnx_export(model.eval(), ([1, 28, 28], [17]))
    onnx_ir.save(exported, path)
    return path
...
def scoring_model(
    ...
    scoring = DIRPATH / "onnx" / f"mnist_vit_{depth}_{layer_norm_type}_scoring.onnx"
    if scoring.is_file():
        return scoring
    
    src = mnist_vit_pruning(depth=depth, layer_norm_type=layer_norm_type)
    onnx.utils.extract_model(
        str(src), str(scoring),
        input_names=["img"], output_names=["mul_1"],
